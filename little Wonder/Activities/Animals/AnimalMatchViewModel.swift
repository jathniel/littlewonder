...
/// Drag an animal token onto the silhouette (shadow) of the same animal. Mirrors
/// `ColorMatchViewModel`'s drag-snap mechanic, matching on `Animal`.
@MainActor
@Observable
final class AnimalMatchViewModel {
    struct Piece: Identifiable {
        let id: String
        let animal: Animal
        /// Center of the drop target (shadow frame) in stage coordinates.
        let target: CGPoint
        ...
        init(id: String, animal: Animal, target: CGPoint, traySlot: CGPoint) {
            self.id = id
            self.animal = animal
            self.target = target
    ...
    static func randomPieces() -> [Piece] {
        ...
        // Distinct animals so each shadow's position maps unambiguously to one animal.
        let animals = Array(Animal.allCases.shuffled().prefix(pieceCount))
