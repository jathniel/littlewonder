...
/// Find every token of the named animal. Each round names a target animal and shows a
/// field of tokens mixing that animal with distractors; the child taps every matching token.
/// Tapping a distractor is ignored (no fail state). Mirrors `ColorFindViewModel`.
@MainActor
@Observable
final class AnimalFindViewModel {
    ...
    /// One target animal per round.
    let targets: [Animal]
    private let fixedRounds: [[Animal]]?
    ...
    var target: Animal { targets[roundIndex] }
    var matchTotal: Int { items.count(where: { $0.animal == self.target }) }
    ...
    init(targets: [Animal]? = nil, rounds: [[Animal]]? = nil) {
        let resolvedTargets = targets ?? Array(Animal.allCases.shuffled().prefix(3))
    ...
    static func makeItems(target: Animal, fixed: [Animal]?) -> [Item] {
        let animals: [Animal]
        if let fixed {
            animals = fixed
        } else {
            let matchCount = Int.random(in: 2...4)
            let others = Animal.allCases.filter { $0 != target }
            ...
            animals = pool.shuffled()
        }
        return animals.enumerated().map { Item(id: $0.offset, animal: $0.element, found: false) }
    }

    /// Marks a matching token found. Tapping a distractor is ignored. Completing the
    /// last round celebrates.
    func tap(_ id: Int) {
        guard let idx = items.firstIndex(where: { $0.id == id }) else { return }
        guard items[idx].animal == target, !items[idx].found else { return }
