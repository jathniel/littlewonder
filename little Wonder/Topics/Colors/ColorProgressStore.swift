...
///
/// Mirrors `NumberProgressStore`: persisted to `UserDefaults`, weekly counters auto-reset
/// when a new ISO week is detected, lifetime counters accumulate forever.
@MainActor
@Observable
final class ColorProgressStore {
    private(set) var weeklyCounts: [ColorActivityID: Int] {
        didSet { encode(weeklyCounts.mappedByRawValue, forKey: Keys.weeklyCounts) }
    }
    private(set) var lifetimeCounts: [ColorActivityID: Int] {
    ...
    var colorsPlayedThisWeek: Int { weeklyCounts.values.reduce(0, +) }
    ...
    init(
        ...
        let storedWeekly: [ColorActivityID: Int] =
            Self.decode([String: Int].self, forKey: Keys.weeklyCounts, from: defaults)?
                .mappedByActivityID() ?? [:]
        let storedLifetime: [ColorActivityID: Int] =
    ...
    private func record(_ activity: ColorActivityID) {
        rolloverIfNeeded()
    ...
    private enum Keys {
        static let weeklyCounts = "colorProgress.weeklyCounts"
        static let lifetimeCounts = "colorProgress.lifetimeCounts"
        static let weekActivity = "colorProgress.weekActivity"
        static let weekAnchor = "colorProgress.weekAnchor"
    }
}

private extension Dictionary where Key == ColorActivityID, Value == Int {
...
private extension Dictionary where Key == String, Value == Int {
    func mappedByActivityID() -> [ColorActivityID: Int] {
        reduce(into: [:]) { partial, pair in
            guard let id = ColorActivityID(rawValue: pair.key) else { return }
