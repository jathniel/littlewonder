...
@MainActor
struct ColorMatchViewModelTests {
    @Test("Initial state — 3 or 4 distinct-colour pieces in tray, none placed")
    func initialState() {
        ...
        let swatches = viewModel.pieces.map(\.swatch)
        #expect(Set(swatches).count == swatches.count) // all distinct
