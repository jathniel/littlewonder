...
@MainActor
struct AnimalMatchViewModelTests {
    @Test("Initial state — 3 or 4 distinct-animal pieces in tray, none placed")
    func initialState() {
        ...
        let animals = viewModel.pieces.map(\.animal)
        #expect(Set(animals).count == animals.count) // all distinct
